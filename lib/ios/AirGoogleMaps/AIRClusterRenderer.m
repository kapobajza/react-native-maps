...
@implementation AIRClusterRenderer {
    ...
    NSMutableDictionary<NSString *, UIImage *> *_cachedMarkerImages;
    LoadImageHandler _loadImageHandler;
}
...
- (instancetype)initWithMapView:(GMSMapView *)mapView
    ...
    if ((self = [super init])) {
        ...
        _cachedMarkerImages = [[NSMutableDictionary<NSString *, UIImage *> alloc] init];
        _zIndex = 1;
    }
    return self;
}
...
- (BOOL)shouldRenderAsCluster:(id<GMUCluster>)cluster atZoom:(float)zoom {
    return cluster.count >= kGMUMinClusterSize && zoom < (_mapView.maxZoom - 2);
}
...
- (void)loadImage:(NSString *)url width:(NSNumber *)width handler:(LoadImageHandler)handler {
    NSURL *imageURL = [NSURL URLWithString:url];
    _loadImageHandler = [handler copy];
...
- (void)renderCluster:(id<GMUCluster>)cluster animated:(BOOL)animated {
    ...
    if ([self shouldRenderAsCluster:cluster atZoom:zoom]) {
        ...
        for (AIRClusterItem* item in cluster.items) {
            ...
            if (iconUrl) {
                UIImage *cachedImage = [_cachedMarkerImages objectForKey:iconUrl];
                
                if (cachedImage) {
                    marker = [self markerWithPosition:item.position
                                                 from:fromPosition
                                             userData:item
                                          clusterIcon:cachedImage
                                             animated:shouldAnimate];
                    
                    [_markers addObject:marker];
                    [_renderedClusterItems addObject:item];
                } else {
                    [self loadImage:iconUrl width:item.iconWidth handler:^(UIImage * _Nonnull image) {
                        marker = [self markerWithPosition:item.position
                                                     from:fromPosition
                                                 userData:item
                                              clusterIcon:image
                        ...
                        [self->_cachedMarkerImages setObject:image forKey:iconUrl];
                        [self->_markers addObject:marker];
                        [self->_renderedClusterItems addObject:item];
                    }];
                }
            } else {
                marker = [self markerWithPosition:item.position
...
- (GMSMarker *)markerWithPosition:(CLLocationCoordinate2D)position
    ...
    if (clusterIcon != nil) {
        ...
        if ([marker.userData conformsToProtocol:@protocol(GMUCluster)]) {
            ...
            AIRClusterItem *item = (AIRClusterItem *)marker.userData;
            
            float newWidth = [item.iconWidth floatValue] / 3;
            float imageWidth = clusterIcon.size.width;
            float imageHeight = clusterIcon.size.height;
            float ratio = newWidth / imageWidth;
            float newHeight = imageHeight * ratio;
            
            imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, newWidth, newHeight)];
